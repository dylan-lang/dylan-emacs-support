...
SWANK> *[]"))
  (with-canonicalized-dime-repl-buffer
    (insert input)
    (dime-check-buffer-contents "Buffer contains input" 
                                 (concat "{}SWANK> [" input "*]"))
    (call-interactively 'dime-repl-return)
    (dime-sync-to-top-level 5)
    (dime-check-buffer-contents "Buffer contains result" result-contents)))

(defun dime-check-buffer-contents (msg expected)
  (let* ((marks '((point . ?*) 
                  (dime-output-start . ?{) (dime-output-end . ?}) 
                  (dime-repl-input-start-mark . ?\[) (point-max . ?\])))
         ...
         (point (point))
         (point-max (point-max)))
    (dime-test-expect (concat msg " [content]") content (buffer-string))
    (macrolet ((test-mark 
                (mark)
                `(when (assoc ',mark marks)
                   (dime-test-expect (format "%s [%s]" msg ',mark)
                                      (cdr (assoc ',mark marks))
                                      ,mark
                                      #'=))))
      (test-mark point)
      (test-mark dime-output-end)
      (test-mark dime-output-start)
      (test-mark dime-repl-input-start-mark)
      (test-mark point-max))))
...
SWANK> "))
  (with-canonicalized-dime-repl-buffer
    ...
    (dime-test-expect "Buffer contains input" 
                       (concat "SWANK> " before after)
                       (buffer-string))
    (call-interactively 'dime-repl-return)
    (dime-sync-to-top-level 5)
    (dime-test-expect "Buffer contains result" 
                       result-contents (buffer-string))))
  
(def-dime-test repl-read
...
SWANK> "))
  (with-canonicalized-dime-repl-buffer
    ...
    (call-interactively 'dime-repl-return)
    (dime-wait-condition "reading" #'dime-reading-p 5)
    (insert input)
    (call-interactively 'dime-repl-return)
    (dime-sync-to-top-level 5)
    (dime-test-expect "Buffer contains result" 
...
SWANK> [*foo]"))
  (with-canonicalized-dime-repl-buffer
    ...
    (dime-sync-to-top-level 5)
    (dime-check-buffer-contents "Buffer contains result" final-contents)))


(def-dime-test interrupt-in-blocking-read
    ()
