...
(defvar dime-current-test)
(defvar dime-unexpected-failures)


;;;;; Execution engine

(defun dime-run-tests ()
  "Run the test suite.
The results are presented in an outline-mode buffer, with the tests
that succeeded initially folded away."
  (interactive)
  (assert (dime-at-top-level-p) () "Pending RPCs or open debuggers.")
  (dime-create-test-results-buffer)
  (unwind-protect
      (let ((dime-repl-history-file 
             (expand-file-name "dime-repl-history" (dime-temp-directory)))
            (dime-tests (if dime-randomize-test-order
                             (dime-shuffle-list dime-tests)
                           dime-tests)))
        (dime-execute-tests))
    (pop-to-buffer dime-test-buffer-name)
    (goto-char (point-min))
    (hide-body)
    ;; Expose failed tests
    (dolist (o (overlays-in (point-min) (point-max)))
      (when (overlay-get o 'dime-failed-test)
        (goto-char (overlay-start o))
        (show-subtree)))))

(defun dime-run-one-test (name)
  "Ask for the name of a test and then execute the test."
  (interactive (list (dime-read-test-name)))
  (let ((test (find name dime-tests :key #'dime-test.name)))
    (assert test)
    (let ((dime-tests (list test)))
      (dime-run-tests))))

(defun dime-read-test-name ()
  (let ((alist (mapcar (lambda (test) 
                         (list (symbol-name (dime-test.name test))))
                       dime-tests)))
...
(defun dime-execute-tests ()
  "Execute each test case with each input.
Return the number of failed tests."
  (save-window-excursion
    (let ((dime-total-tests 0)
          (dime-skipped-tests 0)
      ...
      (dolist (dime-current-test dime-tests)
        (with-struct (dime-test. name (function fname) inputs style) 
            dime-current-test
          (if (and style (not (memq (dime-communication-style) style)))
              (incf dime-skipped-tests)  
            (dime-test-heading 1 "%s" name)
            (dolist (input inputs)
              (incf dime-total-tests)
              (message "%s: %s" name input)
              (dime-test-heading 2 "input: %s" input)
              (if dime-test-debug-on-error
                  ...
                  (condition-case err
                      (apply function input)
                    (error
                     (cond ((dime-test-should-fail-p)
                            (incf dime-expected-failures)
                            (dime-test-failure "ERROR (expected)"
                                                (format "%S" err)))
                           (t
                            (incf dime-unexpected-failures)
                            (dime-print-check-error err))))))))))
      (let ((summary 
             (concat (if (and (zerop dime-expected-failures)
                              (zerop dime-unexpected-failures))
                         (format "All %d tests completed successfully."
                                 dime-total-tests)
                         (format "Failed on %d (%d expected) of %d tests."
                                 (+ dime-expected-failures
                                    dime-unexpected-failures)
                                 dime-expected-failures
                                 dime-total-tests))
                     (if (zerop dime-skipped-tests)
                         ""
                         (format " Skipped %d tests." dime-skipped-tests)))))
        (save-excursion
          (with-current-buffer dime-test-buffer-name
            (goto-char (point-min))
            (insert summary "\n\n")))
        (message "%s" summary)
        dime-unexpected-failures))))

(defun dime-batch-test (results-file &optional test-name randomize)
  "Run the test suite in batch-mode.
Exits Emacs when finished. The exit code is the number of failed tests."
  (let ((dime-test-debug-on-error nil))
    (dime)
    ...
    (let* ((dime-randomize-test-order (when randomize (random t) t))
           (failed-tests (cond (test-name (dime-run-one-test test-name))
                               (t (dime-run-tests)))))
      (with-current-buffer dime-test-buffer-name
        (dime-delete-hidden-outline-text)
        (goto-char (point-min))
        (insert "-*- outline -*-\n\n")
...
(defun dime-create-test-results-buffer ()
  "Create and initialize the buffer for test suite results."
  (ignore-errors (kill-buffer dime-test-buffer-name))
  (with-current-buffer (get-buffer-create dime-test-buffer-name)
    (erase-buffer)
    (outline-mode)
    (set (make-local-variable 'outline-regexp) "\\*+")
    (dime-set-truncate-lines)))

(defun dime-delete-hidden-outline-text ()
...
(defun dime-test-heading (level format &rest args)
...
(defun dime-test-failure (keyword string)
  ...
  (with-current-buffer dime-test-buffer-name
    ...
    (let ((start (point)))
      ...
      (let ((overlay (make-overlay start (point))))
        (overlay-put overlay 'dime-failed-test t)
        (overlay-put overlay 'face 'bold)))
    (insert string "\n")))

(defun dime-test-message (string)
...
(defmacro def-dime-test (name args doc inputs &rest body)
  "Define a test case.
...
BODY is the test case. The body can use `dime-check' to test
conditions (assertions)."
  (multiple-value-bind (name fails-for style)
    ...
    (let ((fname (intern (format "dime-test-%s" name))))
      `(progn
         (defun ,fname ,args
           ,doc
           (dime-sync-to-top-level 0.3)
           ,@body
           (dime-sync-to-top-level 0.3))
         (setq dime-tests 
               (append (remove* ',name dime-tests :key 'dime-test.name)
                       (list (make-dime-test :name ',name :fname ',fname
                                              :fails-for ',fails-for
                                              :style ',style
                                              :inputs ,inputs))))))))
...
BODY returns true if the check succeeds."
  (let ((check-name (gensym "check-name-")))
    `(let ((,check-name ,(typecase test-name
       ...
       (if (progn ,@body)
           (dime-print-check-ok ,check-name)
         (cond ((dime-test-should-fail-p)
                (incf dime-expected-failures)
                (dime-test-failure "FAIL (expected)" ,check-name))
               (t
                (incf dime-unexpected-failures)
                (dime-print-check-failed ,check-name)))
         (when dime-test-debug-on-error
           (debug (format "Check failed: %S" ,check-name)))))))

(defun dime-print-check-ok (test-name)
  (dime-test-message (concat "OK: " test-name)))

(defun dime-print-check-failed (test-name)
  (dime-test-failure "FAILED" test-name))
...
(defun dime-check-top-level (&optional test-name)
  (dime-accept-process-output nil 0.001)
  (dime-check "At the top level (no debugging or pending RPCs)"
    (dime-at-top-level-p)))

(defun dime-at-top-level-p ()
  (and (not (sldb-get-default-buffer))
       (null (dime-rex-continuations))))

(defun dime-wait-condition (name predicate timeout)
...
(defun dime-sync-to-top-level (timeout)
  (dime-wait-condition "top-level" #'dime-at-top-level-p timeout))

;; XXX: unused function
(defun dime-check-sldb-level (expected)
  (let ((sldb-level (when-let (sldb (sldb-get-default-buffer))
                      (with-current-buffer sldb
                        sldb-level))))
    (dime-check ("SLDB level (%S) is %S" expected sldb-level)
      (equal expected sldb-level))))

(defun dime-test-expect (name expected actual &optional test)
...
(defun dime-sldb-level= (level)
  (equal level (sldb-level)))

(defvar dime-test-symbols
  '(("foobar") ("foo@bar") ("@foobar") ("foobar@") ("\\@foobar")
    ("|asdf||foo||bar|")
    ("\\#<Foo@Bar>")
    ("\\(setf\\ car\\)")))

(defun dime-check-symbol-at-point (prefix symbol suffix)
  ...
  (with-temp-buffer
    ...
    (let ((start (point)))
      ...
      (dotimes (i (length symbol))
        (goto-char (+ start i))
        (dime-test-expect (format "Check `%s' (at %d)..."
                                   (buffer-string) (point))
                           symbol
                           (dime-symbol-at-point)
                           #'equal)))))

(def-dime-test symbol-at-point.1 (sym)
    "Check that we can cope with idiosyncratic symbol names."
    dime-test-symbols
  (dime-check-symbol-at-point "" sym ""))

(def-dime-test symbol-at-point.2 (sym)
  "fancy symbol-name _not_ at BOB/EOB"
  dime-test-symbols
  (dime-check-symbol-at-point "(foo " sym " bar)"))

(def-dime-test symbol-at-point.3 (sym)
  "fancy symbol-name with leading ,"
  (remove-if (lambda (s) (eq (aref (car s) 0) ?@)) dime-test-symbols)
  (dime-check-symbol-at-point "," sym ""))

(def-dime-test symbol-at-point.4 (sym)
  "fancy symbol-name with leading ,@"
  dime-test-symbols
  (dime-check-symbol-at-point ",@" sym ""))

(def-dime-test symbol-at-point.5 (sym)
  "fancy symbol-name with leading `"
  dime-test-symbols
  (dime-check-symbol-at-point "`" sym ""))

(def-dime-test symbol-at-point.6 (sym)
  "fancy symbol-name wrapped in ()"
  dime-test-symbols
  (dime-check-symbol-at-point "(" sym ")"))

(def-dime-test symbol-at-point.7 (sym)
  "fancy symbol-name wrapped in #< {DEADBEEF}>"
  dime-test-symbols
  (dime-check-symbol-at-point "#<" sym " {DEADBEEF}>"))

;;(def-dime-test symbol-at-point.8 (sym)
;;  "fancy symbol-name wrapped in #<>"
;;  dime-test-symbols
;;  (dime-check-symbol-at-point "#<" sym ">"))

(def-dime-test symbol-at-point.9 (sym)
  "fancy symbol-name wrapped in #| ... |#"
  dime-test-symbols
  (dime-check-symbol-at-point "#|\n" sym "\n|#"))

(def-dime-test symbol-at-point.10 (sym)
  "fancy symbol-name after #| )))(( |# (1)"
  dime-test-symbols
  (dime-check-symbol-at-point "#| )))(( #|\n" sym ""))

(def-dime-test symbol-at-point.11 (sym)
  "fancy symbol-name after #| )))(( |# (2)"
  dime-test-symbols
  (dime-check-symbol-at-point "#| )))(( #|" sym ""))

(def-dime-test symbol-at-point.12 (sym)
  "fancy symbol-name wrapped in \"...\""
  dime-test-symbols
  (dime-check-symbol-at-point "\"\n" sym "\"\n"))

(def-dime-test symbol-at-point.13 (sym)
  "fancy symbol-name wrapped in \" )))(( \" (1)"
  dime-test-symbols
  (dime-check-symbol-at-point "\" )))(( \"\n" sym ""))

(def-dime-test symbol-at-point.14 (sym)
  "fancy symbol-name wrapped in \" )))(( \" (1)"
  dime-test-symbols
  (dime-check-symbol-at-point "\" )))(( \"" sym ""))

(def-dime-test symbol-at-point.15 (sym)
  "symbol-at-point after #."
  dime-test-symbols
  (dime-check-symbol-at-point "#." sym ""))

(def-dime-test symbol-at-point.16 (sym)
  "symbol-at-point after #+"
  dime-test-symbols
  (dime-check-symbol-at-point "#+" sym ""))


(def-dime-test sexp-at-point.1 (string)
  "symbol-at-point after #'"
  '(("foo")
    ("#:foo")
    ("#'foo")
  ...
  (let ((random-buffer-name (symbol-name (gensym)))
    ...
    (with-temp-buffer
      ...
      (insert (concat "(defun __foo__ (x y)" "\n"
                      "  'nothing)"          "\n"))
      (dotimes (i 100) (insert (format ";;; %d. line\n" (+ 100 i))))
      (dime-check "Checking that newly created buffer is not narrowed."
        (not (dime-buffer-narrowed-p)))

      (goto-char defun-pos)
      (narrow-to-defun)
      (dime-check "Checking that narrowing succeeded."
       (dime-buffer-narrowed-p))

      (dime-with-popup-buffer (random-buffer-name)
        (dime-check ("Checking that we're in Dime's temp buffer `%s'" random-buffer-name)
      ...
      (with-current-buffer random-buffer-name
        ...
        ;; of dime-with-output-to-temp-buffer.
        (dime-popup-buffer-quit t)) 
      (dime-check ("Checking that we've got back from `%s'" random-buffer-name)
        (and (eq (current-buffer) tmpbuffer)
             (= (point) defun-pos)))
      
      (dime-check "Checking that narrowing sustained after quitting Dime's temp buffer."
        (dime-buffer-narrowed-p))
      ...
      (let ((dime-buffer-package "SWANK")
        ...
        (dime-check "Checking that narrowing sustained after M-,"
          (dime-buffer-narrowed-p)))
      )) 
  (dime-check-top-level))
...
confronted with nasty #.-fu."
  ...
  (let ((dime-buffer-package buffer-package))
    (with-temp-buffer
      ...
      (let ((bufname (buffer-name)))
        (dime-edit-definition ".foo.")
        (dime-check ("Definition of `.foo.' is in buffer `%s'." bufname)
          (string= (buffer-name) bufname))
        (dime-check "Definition now at point." (looking-at snippet)))
      )))

(def-dime-test complete-symbol
    ...
    '(("cl:compile" (("cl:compile" "cl:compile-file" "cl:compile-file-pathname"
      ...
      ("swank::compile-file" (("swank::compile-file" 
                              ...
                              "swank::compile-file"))
      ("cl:m-v-l" (nil "")))
  (let ((completions (dime-simple-completions prefix)))
    (dime-test-expect "Completion set" expected-completions completions)))

(def-dime-test arglist
...
Confirm that EXPECTED-ARGLIST is displayed."
  ...
  (let ((arglist (dime-eval `(swank:operator-arglist ,function-name 
                                                      "swank"))))
    (dime-test-expect "Argument list is as expected"
...
Confirm that SUBFORM is correctly located."
  ...
  (with-temp-buffer
    ...
    (let ((font-lock-verbose nil))
      ...
      (dime-compile-string (buffer-string) 1)
      (setq dime-buffer-package ":cl-user")
      (dime-sync-to-top-level 5)
      (goto-char (point-max))
      (dime-previous-note)
      (dime-check error-location-correct
  ...
  (let ((filename "/tmp/dime-tmp-file.lisp"))
    ...
    (let ((cell (cons nil nil)))
      (dime-eval-async
       ...
       (dime-rcurry (lambda (result cell)
                       (setcar cell t)
                       (setcdr cell result))
                     cell))
      (dime-wait-condition "Compilation finished" (lambda () (car cell))
                            0.5)
      (let ((result (cdr cell)))
        (dime-check "Compilation successfull" 
          (eq (dime-compilation-result.successp result) t))))))

(def-dime-test async-eval-debugging (depth)
             ...
             (with-current-buffer (sldb-get-default-buffer)
               (when (> sldb-level debug-hook-max-depth)
                 ...
                 (if (= sldb-level depth)
                   ...
                   ;; Recursively debug.
                   (dime-eval-async '(error))))))))
      (let ((sldb-hook (cons debug-hook sldb-hook)))
        (dime-eval-async '(error))
        (dime-sync-to-top-level 5)
        (dime-check ("Maximum depth reached (%S) is %S."
                      debug-hook-max-depth depth)
          (= debug-hook-max-depth depth))))))

(def-dime-test unwind-to-previous-sldb-level (level2 level1)
  ...
  (lexical-let ((level2 level2)
    ...
    (let ((debug-hook
               ...
               (ecase state
                 (enter
                  (cond ((= sldb-level level2)
                        ...
                        (t
                         (dime-eval-async `(cl:aref cl:nil ,sldb-level)))))
                 (leave
      ...
      (let ((sldb-hook (cons debug-hook sldb-hook)))
        (dime-eval-async `(cl:aref cl:nil 0))
        (dime-sync-to-top-level 15)
        (dime-check-top-level)
        (dime-check ("Maximum depth reached (%S) is %S." max-depth level2)
          (= max-depth level2))
        (dime-check ("Final state reached.")
          (eq state 'ok))))))
...
(def-dime-test loop-interrupt-quit
    ()
    "Test interrupting a loop."
    '(())
  (dime-check-top-level)
  (dime-eval-async '(cl:loop) (lambda (_) ) "CL-USER")
  (dime-accept-process-output nil 1)
  (dime-check "In eval state." (dime-busy-p))
  (dime-interrupt)
  (dime-wait-condition "First interrupt" (lambda () (dime-sldb-level= 1)) 5)
  (with-current-buffer (sldb-get-default-buffer) 
    (sldb-quit))
  (dime-sync-to-top-level 5)
  (dime-check-top-level))

(def-dime-test loop-interrupt-continue-interrupt-quit
    ()
  ...
  (dime-eval-async '(cl:loop) (lambda (_) ) "CL-USER")
  (sleep-for 1)
  (dime-wait-condition "running" #'dime-busy-p 5)
  (dime-interrupt)
  (dime-wait-condition "First interrupt" (lambda () (dime-sldb-level= 1)) 5)
  (with-current-buffer (sldb-get-default-buffer)
    (sldb-continue))
  (dime-wait-condition "running" (lambda () 
                                    (and (dime-busy-p)
                                         (not (sldb-get-default-buffer)))) 5)
  (dime-interrupt)
  (dime-wait-condition "Second interrupt" (lambda () (dime-sldb-level= 1)) 5)
  (with-current-buffer (sldb-get-default-buffer)
    (sldb-quit))
  (dime-sync-to-top-level 5)
  (dime-check-top-level))
 
(def-dime-test interactive-eval 
    ()
  ...
  (lexical-let ((done nil))
    (let ((sldb-hook (lambda () (sldb-continue) (setq done t))))
      ...
      (while (not done) (dime-accept-process-output))
      (dime-sync-to-top-level 5)
      (dime-check-top-level)
      (unless noninteractive
        (let ((message (current-message)))
          (dime-check "Minibuffer contains: \"3\""
            (equal "=> 3 (2 bits, #x3, #o3, #b11)" message)))))))

(def-dime-test interrupt-bubbling-idiot 
    ()
    "Test interrupting a loop that sends a lot of output to Emacs."
    '(())
  (dime-accept-process-output nil 1)
  (dime-check-top-level)
  (dime-eval-async '(cl:loop :for i :from 0 :do (cl:progn (cl:print i) 
                                                           (cl:finish-output)))
                    (lambda (_) ) 
                    "CL-USER")
  (sleep-for 1)
  (dime-interrupt)
  (dime-wait-condition "Debugger visible" 
                        (lambda () 
                          (and (dime-sldb-level= 1)
...
(def-dime-test (interrupt-encode-message (:style :sigio))
    ()
    "Test interrupt processing during swank::encode-message"
    '(())
  (dime-eval-async '(cl:loop :for i :from 0 
                              :do (swank::background-message "foo ~d" i)))
  (sleep-for 1)
  (dime-eval-async '(cl:/ 1 0))
  (dime-wait-condition "Debugger visible" 
                        (lambda () 
                          (and (dime-sldb-level= 1)
                               (get-buffer-window (sldb-get-default-buffer))))
...
(def-dime-test inspector
    ...
    '(((let ((h (make-hash-table)))
      ...
      ('cons)
      (#'cons))
  (dime-inspect (prin1-to-string exp))
  (assert (not (dime-inspector-visible-p)))
  (dime-wait-condition "Inspector visible" #'dime-inspector-visible-p 5)
  (with-current-buffer (window-buffer (selected-window))
    (dime-inspector-quit))
  (dime-wait-condition "Inspector closed" 
                        (lambda () (not (dime-inspector-visible-p)))
                        5)
  (dime-sync-to-top-level 1))

(defun dime-buffer-visible-p (name)
  (let ((buffer (window-buffer (selected-window))))
    (string-match name (buffer-name buffer))))

(defun dime-inspector-visible-p ()
  (dime-buffer-visible-p (dime-buffer-name :inspector)))

(defun dime-execute-as-command (name)
...
(def-dime-test macroexpand 
  ...
  (with-temp-buffer
    ...
    (insert bufcontent)
    (goto-char (point-min))
    (dime-execute-as-command 'dime-macroexpand-1)
    (dime-wait-condition "Macroexpansion buffer visible" 
                          (lambda () 
                            (dime-buffer-visible-p 
                             (dime-buffer-name :macroexpansion)))
                          5)
    (with-current-buffer (get-buffer (dime-buffer-name :macroexpansion))
      (dime-test-expect "Initial macroexpansion is correct"
                         expansion1 
                         (downcase (buffer-string)))
      (search-forward search-str)
      (backward-up-list)
      (dime-execute-as-command 'dime-macroexpand-1-inplace)
      (dime-sync-to-top-level 3)
      (dime-test-expect "In-place macroexpansion is correct"
                         expansion2 
                         (downcase (buffer-string)))
      (dime-execute-as-command 'dime-macroexpand-undo)
      (dime-test-expect "Expansion after undo is correct"
                         expansion1
                         (downcase (buffer-string)))))
    (setq dime-buffer-package ":cl-user"))

(def-dime-test indentation (buffer-content point-markers)
...
"
  ...
  (with-temp-buffer
    ...
    (dime-sync-to-top-level 3)
    (dime-update-indentation)
    (dime-sync-to-top-level 3)
    (dolist (marker point-markers)
      (search-backward marker)
      (beginning-of-defun)
      (indent-sexp))
    (dime-test-expect "Correct buffer content"
                       buffer-content
...
(def-dime-test break
  ...
  (dime-accept-process-output nil 0.2)
  (dime-check-top-level)
  (dime-eval-async 
  ...
  (dotimes (i times)
    ...
    (with-current-buffer (sldb-get-default-buffer)
      (sldb-continue))
    (dime-wait-condition "sldb closed" 
                          (lambda () (not (sldb-get-default-buffer)))
                          0.2))
  (dime-sync-to-top-level 1))

(def-dime-test (break2 (:fails-for "cmucl" "allegro" "ccl"))
    (times exp)
...
on *DEBUGGER-HOOK*."
  ...
  (dime-test-break times exp))

(def-dime-test locally-bound-debugger-hook
    ()
   ...
   (prin1-to-string `(defun cl-user::quux ()
                       (block outta
                         (let ((*debugger-hook*
                           ...
                           (error "FOO")))))
   0)
  (dime-sync-to-top-level 2)
  (dime-eval-async '(cl-user::quux))
  ;; FIXME: dime-wait-condition returns immediately if the test returns true
  (dime-wait-condition "Checking that Debugger does not popup" 
                        (lambda () 
                          (not (sldb-get-default-buffer)))
                        3)
  (dime-sync-to-top-level 5))

(def-dime-test end-of-file 
    (expr)
  ...
  (let ((value (dime-eval 
    ...
    (dime-test-expect "Debugger invoked" "END-OF-FILE" value)))
                      
(def-dime-test interrupt-at-toplevel
    ()
    "Let's see what happens if we send a user interrupt at toplevel."
    '(())
  (dime-check-top-level)
  (unless (and (eq (dime-communication-style) :spawn)
               (not (featurep 'dime-repl)))
    (dime-interrupt)
    (dime-wait-condition 
     "Debugger visible" 
     (lambda () 
...
(def-dime-test interrupt-in-debugger (interrupts continues)
...
CONTINUES  ... how often the continue restart should be invoked"
    '((1 0) (2 1) (4 2))
  (dime-check "No debugger" (not (sldb-get-default-buffer)))
  (when (and (eq (dime-communication-style) :spawn)
             (not (featurep 'dime-repl)))
    (dime-eval-async '(swank::without-dime-interrupts
                        (swank::receive)))
    (sit-for 0.2))
  (dotimes (i interrupts)
    (dime-interrupt)
    (let ((level (1+ i)))
      (dime-wait-condition (format "Debug level %d reachend" level)
  ...
  (dotimes (i continues)
    ...
    (let ((level (- interrupts (1+ i))))
      (dime-wait-condition (format "Return to debug level %d" level)
                            (lambda () (equal (sldb-level) level))
                            2)))
  (with-current-buffer (sldb-get-default-buffer)
    (sldb-quit))
...
(def-dime-test (disconnect-one-connection (:style :spawn)) ()
    "`dime-disconnect' should disconnect only the current connection"
    '(())
  (let ((connection-count (length dime-net-processes))
        (old-connection dime-default-connection)
        (dime-connected-hook nil))
    (unwind-protect
         (let ((dime-dispatching-connection 
                (dime-connect "localhost" 
                               ;; Here we assume that the request will
                               ;; be evaluated in its own thread.
                               (dime-eval `(swank:create-server 
                                             :port 0 ; use random port
                                             :style :spawn
                                             :dont-close nil)))))
           (dime-sync-to-top-level 3)
           (dime-disconnect)
           (dime-test-expect "Number of connections must remane the same"
                              connection-count
                              (length dime-net-processes)))
      (dime-select-connection old-connection))))

(def-dime-test disconnect-and-reconnect
    ()
    "Close the connetion.
...
Reconnect afterwards."
    '(())
  (dime-check-top-level)
  (let* ((c (dime-connection))
         (p (dime-inferior-process c)))
...
;; XXX: unused function
(defun dime-intersperse (element list)
